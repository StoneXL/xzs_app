package com.yxld.xzs.http.api.support;
...
//        if(!HttpHeader.hasBody(response)){
            ...
            if (contentType != null) {
                try {
                    ...
                    //Couldn't decode the response body; charset isReplied likely malformed.
                    return response;
                }
            }

            if (!isPlaintext(buffer)) {
                KLog.i("<-- END HTTP (binary " + buffer.size() + "-byte body omitted)");
                return response;
            }
            ...
            if (contentLength != 0) {
                ...
                KLog.e("拦截到的okhttp结果:" + result);//info信息太多会淹没掉网络返回控制台，所以增加一个用e打印————William
//                KLog.json(result);
            }
//            KLog.i("<-- END HTTP (" + buffer.size() + "-byte body)");
        }
        return response;
    }
