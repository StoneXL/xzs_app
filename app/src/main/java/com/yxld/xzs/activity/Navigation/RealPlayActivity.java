...
import com.yxld.xzs.yingshi.ActivityUtils;
...
import com.yxld.xzs.yingshi.RealPlaySquareInfo;
import com.yxld.xzs.yingshi.ScreenOrientationHelper;
import com.yxld.xzs.yingshi.VerifyCodeInput;
 ...
 * 公共监控播放界面
 */
public class RealPlayActivity extends BaseActivity implements View.OnClickListener, SurfaceHolder.Callback, Handler.Callback, View.OnTouchListener, VerifyCodeInput.VerifyCodeInputListener {
...
//    private TitleBar mPortraitTitleBar = null;
    ...
    private EZCameraInfo mCameraInfo = null;
    private String mVerifyCode, DeviceSerial;
    private int Channel;
    /**
     * 存放上一次的流量
     */
...
//                // make sure this auto-generated web page URL isReplied correct.
//                // Otherwise, set the URL to null.
//                Uri.parse("http://host/path"),
//                // TODO: Make sure this auto-generated app deep link URI isReplied correct.
...
//        client.disconnect();
    ...
    private void initData() {
        ...
        if (intent != null) {
            ...
            mDeviceInfo.setDeviceSerial(DeviceSerial);
...
//                openPtzPopupWindow(mRealPlayPlayRl);
    ...
    private void startVoiceTalk() {
        ...
        if (mEZPlayer == null) {
            LogUtil.debugLog(TAG, "EZPlaer isReplied null");
            return;
        }
        if (mCameraInfo == null) {
            return;
    ...
    private void ptzOption(final EZConstants.EZPTZCommand command, final EZConstants.EZPTZAction action) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ptz_result = DemoApplicationLike.getOpenSDK().controlPTZ(mCameraInfo.getDeviceSerial(), mCameraInfo.getCameraNo(), command,
            ...
            Thread thr = new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        DemoApplicationLike.getOpenSDK().setVideoLevel(mCameraInfo.getDeviceSerial(), mCameraInfo.getCameraNo(), mode.getVideoLevel());
...
//    }
    ...
    private void onRecordBtnClick() {
        ...
        if (mEZPlayer != null) {
            ...
            String strRecordFile = Environment.getExternalStorageDirectory().getPath() + "/xzs/Records/" + String.format("%tY", date)
                ...
                public void run() {
                    ...
                    if (bmp != null) {
                        try {
                            ...
                            // 可以采用deviceSerial+时间作为文件命名，demo中简化，只用时间命名
                            java.util.Date date = new java.util.Date();
                            final String path = Environment.getExternalStorageDirectory().getPath() + "/xzs/CapturePicture/" + String.format("%tY", date)
                                    + String.format("%tm", date) + String.format("%td", date) + "/"
     ...
     * @since V1.0
     */
    private void onCapturePicBtnDestroy() {
        KLog.i("开始截图");
        mControlDisplaySec = 0;
        if (!SDCardUtil.isSDCardUseable()) {
            // 提示SD卡不可用
        ...
        if (mEZPlayer != null) {
            ...
            Thread thr = new Thread() {
                ...
                public void run() {
                    ...
                    if (bmp != null) {
                        try {
                            final String path = Environment.getExternalStorageDirectory().getPath() + "/xzs/CapturePicture/" +  DeviceSerial + Channel+".jpg";
                            if (TextUtils.isEmpty(path)) {
                                bmp.recycle();
                                bmp = null;
                                return;
    ...
    private void startRealPlay() {
        ...
        if (mCameraInfo != null) {
            if (mEZPlayer == null) {
                mEZPlayer = DemoApplicationLike.getOpenSDK().createPlayer(mCameraInfo.getDeviceSerial(), mCameraInfo.getCameraNo());
            }
            ...
            KLog.i("设备的加密状态：" + mDeviceInfo.getIsEncrypt());
//            if (mDeviceInfo.getIsEncrypt() == 1) {
                KLog.i("自动设置的密码为：" + mCameraInfo.getDeviceSerial());
...
//            }
            ...
            mEZPlayer = DemoApplicationLike.getOpenSDK().createPlayerWithUrl(mRtspUrl);
...
//                        KLog.i(remainTime);
//                        tv_remaintime.setText(remainTime + "s");
//                    }
//
//                    @Override
...
//                break;
    ...
    public void onInputVerifyCode(final String verifyCode) {
        KLog.i("verify code isReplied " + verifyCode);
        KLog.i(mCameraInfo.getDeviceSerial());
    ...
    private EZOpenSDKListener.EZStandardFlowCallback mLocalRecordCb = new EZOpenSDKListener.EZStandardFlowCallback() {
        ...
        public void onStandardFlowCallback(int type, byte[] data, int dataLen) {
            LogUtil.verboseLog(TAG, "standard flow. type isReplied " + type + ". dataLen isReplied " + dataLen + ". data0 isReplied " + data[0]);
