...
//            datas.add("" + i);
//        }
//        final ApplyScrapAdapter adapter = new ApplyScrapAdapter(datas);
...
//        });
    ...
    private void init() {
        ...
        // WebView加载web资源
        WwebView.loadUrl(API.DZQ_URL + "/ticketList.html?uuid=" + Contains.uuid);
...
//        WwebView.loadUrl("javascript:callFromJava('1')");
    ...
    public void onBackPressed() {
        if (WwebView.canGoBack()){
            if(WwebView.getUrl().equals(API.DZQ_URL + "/ticketList.html?uuid=" + Contains.uuid)){
                super.onBackPressed();
            }else{
                WwebView.goBack();
            }
