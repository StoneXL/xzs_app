...
import com.yxld.xzs.activity.camera.RecordFilesActivity;
...
public class SettingListener implements ISetting {
    ...
    public void vRetGetRemoteDefenceResult(String contactId, int state) {
        Log.d("...", "vRetGetRemoteDefenceResult: " + contactId + "======" + state);
    }

    @Override
    public void vRetGetRemoteRecordResult(int state) {
        Log.d("...", "vRetGetRemoteRecordResult: " + state);
        EventBus.getDefault().post("GetRemoteRecordResult" + state);
    }
    ...
    public void vRetGetRecordFiles(String[] names, byte option0, byte option1) {
        ...
        DemoApplicationLike.getInstance().sendBroadcast(i);
    ...
    public void vRetCustomCmd(int contactId, int len, byte[] cmd) {
        ...
        Toast.makeText(DemoApplicationLike.getInstance(), "" + contactId, Toast.LENGTH_LONG).show();
    }
    ...
    public void vRetGetIndexFriendStatus(int count, String[] contactIds, int[] IdProtery, int[] status, int[] DevTypes, int[] SubType, int[] DefenceState, byte bRequestResult) {
        Log.d("...", "vRetGetIndexFriendStatus: count" + count);
        for (int i = 0; i < contactIds.length; i++) {
            Log.d("...", "vRetGetIndexFriendStatus: contactIds" + contactIds[i]);
        }
        for (int i = 0; i < IdProtery.length; i++) {
            Log.d("...", "vRetGetIndexFriendStatus: IdProtery" + IdProtery[i]);
        }
        for (int i = 0; i < status.length; i++) {
            Log.d("...", "vRetGetIndexFriendStatus: status" + status[i]);
        }
        for (int i = 0; i < DevTypes.length; i++) {
            Log.d("...", "vRetGetIndexFriendStatus: DevTypes" + DevTypes[i]);
        }
        for (int i = 0; i < SubType.length; i++) {
            Log.d("...", "vRetGetIndexFriendStatus: SubType" + SubType[i]);
        }
        for (int i = 0; i < DefenceState.length; i++) {
            Log.d("...", "vRetGetIndexFriendStatus: DefenceState" + DefenceState[i]);
        }
        Log.d("...", "vRetGetIndexFriendStatus: bRequestResult" + bRequestResult);
