 ...
 */
public class ErrorTransformer2<T> implements Observable.Transformer<BaseResultEntity3<T>, T>{

    @Override
    public Observable<T> call(Observable<BaseResultEntity3<T>> responseObservable) {
        return responseObservable.map(new Func1<BaseResultEntity3<T>, T>() {
            @Override
            public T call(BaseResultEntity3<T> httpResult) {
...
//                if (httpResult.getCode() != 200) throw new RuntimeException(httpResult.getMessage());
                ...
                Contains.uuid = httpResult.getUuid();
    ...
    public static <T> ErrorTransformer2<T> create() {
        return new ErrorTransformer2<>();
    }

    private static ErrorTransformer2 instance = null;
     ...
     * 双重校验锁单例(线程安全)
     */
    public static <T>ErrorTransformer2<T> getInstance() {
        if (instance == null) {
            synchronized (ErrorTransformer2.class) {
                if (instance == null) {
                    instance = new ErrorTransformer2<>();
                }
            }
        }
        return instance;
